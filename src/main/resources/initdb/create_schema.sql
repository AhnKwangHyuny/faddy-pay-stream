CREATE DATABASE IF NOT EXISTS `faddy_labs`;
USE faddy_labs;
CREATE USER IF NOT EXISTS `faddy`@`localhost` IDENTIFIED BY 'agh@p970314';
CREATE USER `faddy`@`%` IDENTIFIED BY 'testlabs';
GRANT all privileges ON `faddy_labs`.* TO `faddy`@`localhost`;
GRANT all privileges ON `faddy_labs`.* TO `faddy`@`%`;
